...
| O-002-F | **Manager Bypass amendment (OWNER-DIRECTED, July 2026):** the Elytra Manager Authorization adds manager-level downward price authority inside the deck's Investment slide (PIN-gated ceremony). Honesty deltas applied by CTO and accepted into the build: (1) the "3 per month" scarcity is REAL — device-enforced counter, blocks at zero, display always true (a fabricated counter was REJECTED under DOC-2/no-fake-scarcity doctrine); (2) every adjustment is DISCLOSED on the homeowner's printed report and PDF ("manager-authorized pricing adjustment," with the standard quote shown); (3) advisory floor warning below 20% under band price; (4) client-side PIN is ceremony, not security. "No discounts ever" is hereby amended to "no discounts except documented manager authorizations, max 3/month/device." | Owner directive over CTO objection to unlimited undisclosed discounting; conflicts noted and resolved as above |
